...
def measure_execution_time(fn_name, func, *args, **kwargs) -> float:
    num_runs = 10
    ...
    print(
        f'{fn_name} took an average of {average_execution_time:.6f} seconds '
        f'to execute ({len(args[0])} items).'
    )
    return average_execution_time


def to_md_table(list_of_dicts: list[dict[str, str | int]]) -> str:
    markdowntable = ''
    markdownheader = '| ' + ' | '.join(map(str, list_of_dicts[0].keys())) + ' |'
    markdownheaderseparator = '|-----' * len(list_of_dicts[0].keys()) + '|'
...
if __name__ == '__main__':
    ...
    string_length = 15
    example_datas = [
        generate_random_strings(random_seed, rep, string_length)
        for rep in [10, 100, 1_000, 10_000, 100_000, 1_000_000]
    ]

    benchmark_results = []
    for example_data in example_datas:
        dur_py = measure_execution_time('natsort_py', natsort_py, example_data)
        dur_rs = measure_execution_time(
            'natsort_rs', natsort_rs, example_data, None, False, False
        )
        dur_rs_par = measure_execution_time(
            'natsort_rs_par', natsort_rs, example_data, None, False, True
        )
    ...
    print(to_md_table(benchmark_results))
